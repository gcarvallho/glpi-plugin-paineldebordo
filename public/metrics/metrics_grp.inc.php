...
switch ($sel_period) {
	...
	case 1:
		...
		$period_name = __('Current year', 'paineldebordo');
		break;
	case 2:
		...
		$period_name = __('Current month', 'paineldebordo');
		break;
	case 3:
	...
	case 4:
		...
		$period_name = __('Last 15 days', 'paineldebordo');
		break;
	case 5:
		...
		$period_name = __('Last 30 days', 'paineldebordo');
		break;
	case 6:
		...
		$period_name = __('Last 90 days', 'paineldebordo');
		break;
	case 7:
		...
		$period_name = __('Last 180 days', 'paineldebordo');
		break;
	default:
		$period = '';
		$periodp = '';
...
switch (date("m")) {
	case "01":
		$mes = __('January', 'paineldebordo');
		break;
	case "02":
		$mes = __('February', 'paineldebordo');
		break;
	case "03":
		$mes = __('March', 'paineldebordo');
		break;
	case "04":
		$mes = __('April', 'paineldebordo');
		break;
	case "05":
		$mes = __('May', 'paineldebordo');
		break;
	case "06":
		$mes = __('June', 'paineldebordo');
		break;
	case "07":
		$mes = __('July', 'paineldebordo');
		break;
	case "08":
		$mes = __('August', 'paineldebordo');
		break;
	case "09":
		$mes = __('September', 'paineldebordo');
		break;
	case "10":
		$mes = __('October', 'paineldebordo');
		break;
	case "11":
		$mes = __('November', 'paineldebordo');
		break;
	case "12":
		$mes = __('December', 'paineldebordo');
		break;
}

switch (date("w")) {
	case "0":
		$dia = __('Sunday', 'paineldebordo');
		break;
	case "1":
		$dia = __('Monday', 'paineldebordo');
		break;
	case "2":
		$dia = __('Tuesday', 'paineldebordo');
		break;
	case "3":
		$dia = __('Wednesday', 'paineldebordo');
		break;
	case "4":
		$dia = __('Thursday', 'paineldebordo');
		break;
	case "5":
		$dia = __('Friday', 'paineldebordo');
		break;
	case "6":
		$dia = __('Saturday', 'paineldebordo');
		break;
}

// time period for metrics
$sql_met = "SELECT value FROM glpi_plugin_paineldebordo_config WHERE name = 'metric' AND users_id = " . $_SESSION['glpiID'] . "";
$result_met = $DB->doQuery($sql_met);
...
$resultm = $DB->doQuery($querym) or die('erro');
...
$result_hoje = $DB->doQuery($sql_hoje);
...
$resultd = $DB->doQuery($queryd) or die('erro_day');
...
$result2 = $DB->doQuery($query2) or die('erro');
...
$w = $conta_cons - $v;
$gauge_val = round(($w * 100) / ($conta_cons == 0 ? 1 : $conta_cons), 1);
...
$result_sta = $DB->doQuery($query_sta) or die('erro_stat');
...
$res_type = $DB->doQuery($query_type);
...
$res_prob = $DB->doQuery($query_prob);
...
$result_staty = $DB->doQuery($query_staty);
