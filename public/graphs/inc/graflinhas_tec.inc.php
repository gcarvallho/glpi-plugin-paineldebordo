...
if ($interval >= "31") {
	...
	while ($row_result = $DB->fetchAssoc($resultd)) {
		...
		$resultm = $DB->doQuery($querym) or die('erro m');
...
if ($interval >= "31") {
	...
	$resultf = $DB->doQuery($queryf) or die('erro f');
...
if ($interval >= "31") {
	...
	$resultb = $DB->doQuery($queryb) or die('erro b');
...
$(function ()
{
        $('#graf_linhas').highcharts({
          ...
          series: [{
                ...
                name: '" . __('Opened', 'paineldebordo') . " (" . array_sum($quant_o) . ")',
                data: [$quant_o2] },

                {
                name: '" . __('Closed', 'paineldebordo') . " (" . array_sum($quant_c) . ")',
                data: [$quant_c2] },
                
                {
                name: '" . __('Backlog', 'paineldebordo') . " (" . end($arr_bk) . ")',
                type: 'spline',
