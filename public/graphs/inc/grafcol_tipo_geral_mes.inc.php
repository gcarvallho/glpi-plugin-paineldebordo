...
// all months
$months = $DB->doQuery($querya) or die('erro');
...
while ($row_result = $DB->fetchAssoc($months)) {
	...
	$months2 = $DB->doQuery($querya2) or die('erro a');
...
$months = $DB->doQuery($querys) or die('erro');
...
while ($row_result = $DB->fetchAssoc($months)) {
	...
	$months2 = $DB->doQuery($querys2) or die('erro a');
	$row_result2 = $DB->fetchAssoc($months2);
...
$months = $DB->doQuery($queryp) or die('erro');
...
while ($row_result = $DB->fetchAssoc($months)) {
	...
	$months2 = $DB->doQuery($queryp2) or die('erro a');
	$row_result2 = $DB->fetchAssoc($months2);
...
$(function () {
        $('#graf_tipo').highcharts({
            ...
            title: {
                text: '" . __('Tickets', 'paineldebordo') . " " . __('by Type', 'paineldebordo') . "'
            },
            xAxis: {
                categories: $grfp3
            },
