...
if ($interval <= "31") {
	...
	$resultd = $DB->doQuery($queryd) or die('erro');
...
if ($interval >= "31") {
	...
	$resulta = $DB->doQuery($querya) or die('erro');
} else {
...
if ($interval >= "31") {
	...
	while ($row_result = $DB->fetchAssoc($resulta)) {
		...
		$resulta2 = $DB->doQuery($querya2) or die('erro a');
...
if ($interval >= "31") {
	...
	$results = $DB->doQuery($querys) or die('erro');
	...
	while ($row_result = $DB->fetchAssoc($resultd)) {
		...
		$results = $DB->doQuery($querys) or die('erro s' . $DB->error());
...
if ($interval >= "31") {
	...
	$resultf = $DB->doQuery($queryf) or die('erro');
	...
	while ($row_result = $DB->fetchAssoc($resultd)) {
		...
		$resultf = $DB->doQuery($queryf) or die('erro f' . $DB->error());
	...
	echo  "
            yAxis: [{
                ...
                title: { // Primary yAxis
          		...
          		{ // Secondary yAxis
                title: {
                    text: '" . __('Satisfaction', 'paineldebordo') . "',
                    style: {
                       // color: '#4572A7'
                    }
                },
...
if (array_sum($quantsat) != 0) {
	...
	echo  "
					{ // satisfacao
                name: '" . __('Satisfaction', 'paineldebordo') . " (" . $satisf . ")',
...
echo "
          		 {
                name: '" . __('Opened', 'paineldebordo') . " (" . $opened . ")',
                ...
                data: [$quantm2]
    				...
    				{
                name: '" . __('Solved', 'paineldebordo') . " (" . $solved . ")',
                dataLabels: {
                    enabled: false
                },
                data: [$quants2]
                ...
                {
                name: '" . __('Late', 'paineldebordo') . " (" . $late . ")',
                ...
                {
                name: '" . __('Closed', 'paineldebordo') . " (" . $closed . ")',
                dataLabels: {
                    enabled: false
                    },
                data: [$quantf2]
