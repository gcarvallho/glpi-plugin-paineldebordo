...
if (!isset($_POST["sel_group"])) {
	$id_grp = $_REQUEST["sel_group"] ?? 0;
} else {
	$id_grp = $_POST["sel_group"];
}

if (!empty($id_grp)) {
	plugin_paineldebordo_assertGroupAccess((int) $id_grp);
}
...
<head>
	<title> GLPI - <?php echo __('Tickets', 'paineldebordo') . '  ' . __('by Group', 'paineldebordo') ?> </title>
			...
			<div id="charts" class="fluid chart">
				<div id="pad-wrapper">
					<div id="head-lg" class="fluid">
						...
						<div id="titulo_rel"> <?php echo __('Tickets', 'paineldebordo') . '  ' . __('by Group', 'paineldebordo') ?> </div>
						<div id="datas-tec" class="span12 fluid">
							...
							ORDER BY `name` ASC";
											...
											$result_grp = $DB->doQuery($sql_grp);
											$grp = $DB->fetchAssoc($result_grp);

											$res_grp = $DB->doQuery($sql_grp);
											$arr_grp = array();
											$arr_grp[0] = "-- " . __('Select a group', 'paineldebordo') . " --";
					...
					if (!isset($_POST['con'])) {
						...
						if ($con == "1") {
							...
							if ($id_grp == 0) {
								echo '<script language="javascript"> alert(" ' . __('Select a group', 'paineldebordo') . ' "); </script>';
...
<table class='fluid'  style='width:100%; font-size: 18px; font-weight:bold;' cellpadding = 1px>
		<td  style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> " . __('Group', 'paineldebordo') . ": </span>" . $grp_name['name'] . " </td>
		<td  style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> " . __('Tickets', 'paineldebordo') . ": </span>" . $consulta . " </td>
		<td colspan='3' style='font-size: 16px; font-weight:bold; vertical-align:middle; width:200px;'><span style='color:#000;'>
		" . __('Period', 'paineldebordo') . ": </span> " . conv_data($data_ini2) . " a " . conv_data($data_fin2) . "
		</td>
...
<table align='right' style='margin-bottom:10px;'>
		<tr>
			<td>
				<button class='btn btn-primary btn-sm' type='button' name='abertos' value='Abertos' onclick='location.href=\"rel_grupo.php?con=1&stat=open&sel_group=" . $id_grp . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Opened', 'paineldebordo') . " </button>
				<button class='btn btn-primary btn-sm' type='button' name='fechados' value='Fechados' onclick='location.href=\"rel_grupo.php?con=1&stat=close&sel_group=" . $id_grp . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Closed', 'paineldebordo') . " </button>
				<button class='btn btn-primary btn-sm' type='button' name='todos' value='Todos' onclick='location.href=\"rel_grupo.php?con=1&stat=all&sel_group=" . $id_grp . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('All', 'paineldebordo') . " </button>
			</td>
		</tr>
</table>
													...
													buttons: ["csvHtml5", "excelHtml5",
														{
															...
															message: "<?php echo __('Group'); ?> : <?php echo $grp_name['name'] . '  -  '; ?> <?php echo  __('Tickets', 'paineldebordo'); ?> : <?php echo $consulta . '  -  '; ?><?php echo  __('Period', 'paineldebordo'); ?> : <?php echo conv_data($data_ini2); ?> a <?php echo conv_data($data_fin2); ?> ",
														}
													]
												}
											]
