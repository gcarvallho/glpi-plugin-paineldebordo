...
<head>
	<title> GLPI - <?php echo __('Tickets', 'paineldebordo') . '  ' . __('by Technician', 'paineldebordo') ?> </title>
...
<body style="background-color: #e5e5e5; margin-left:0%;">
	...
	<?php
	# entity
	$sql_e = "SELECT value FROM glpi_plugin_paineldebordo_config WHERE name = 'entity' AND users_id = " . $_SESSION['glpiID'] . "";
	$result_e = $DB->doQuery($sql_e);
...
ORDER BY name ASC ";
			...
			<div id="charts" class="fluid chart">
				<div id="pad-wrapper">
					<div id="head-rel" class="fluid">
						...
						<div id="titulo_rel"> <?php echo __('Tickets', 'paineldebordo') . '  ' . __('by Technician', 'paineldebordo') ?> </div>
						...
						<tr>
							...
							<td>&nbsp;</td>
										...
										<td style="margin-top:2px;">
											...
											// lista de técnicos
											$res_tec = $DB->doQuery($sql_tec);
											$arr_tec = array();
											$arr_tec[0] = "-- " . __('Select a technician', 'paineldebordo') . " --";
									...
									<tr>
										<td colspan="2" align="center">
											<button class="btn btn-primary btn-sm" type="submit" name="submit" value="Atualizar"><i class="fa fa-search"></i>&nbsp; <?php echo __('Consult', 'paineldebordo'); ?></button>
											<button class="btn btn-primary btn-sm" type="button" name="Limpar" value="Limpar" onclick="location.href='<?php echo $url2 ?>'"> <i class="fa fa-trash-o"></i>&nbsp; <?php echo __('Clean', 'paineldebordo'); ?> </button>
										</td>
										</td>
									</tr>
					...
					if (isset($_GET['con'])) {
						...
						if ($con == "1") {
							...
							if ($id_tec == 0) {
								echo '<script language="javascript"> alert(" ' . __('Select a technician', 'paineldebordo') . ' "); </script>';
	...
	ORDER BY id DESC ";
							...
							$result_cham = $DB->doQuery($sql_cham);
	...
	ORDER BY id DESC ";
							...
							$result_cons1 = $DB->doQuery($consulta1);
	...
	ORDER BY id DESC  ";
								...
								$result_ab = $DB->doQuery($sql_ab) or die("erro_ab");
	...
	" . $entidade . " ";
								...
								$result_sat = $DB->doQuery($query_sat) or die('erro');
	...
	WHERE `id` = " . $id_tec . " ";
								...
								$result_nome = $DB->doQuery($sql_nome);
	...
	AND glpi_tickets_users.tickets_id = glpi_tickets.id ";
									...
									$result_stat = $DB->doQuery($query_stat);
	...
	AND glpi_tickets_users.tickets_id = glpi_tickets.id ";
									...
									$result_stat_c = $DB->doQuery($query_stat_c);
	...
	AND glpi_tickets_users.tickets_id = glpi_tickets.id ";
									...
									$result_stat_s = $DB->doQuery($query_stat_s);
	...
	<table class='fluid'  style='width:100%; font-size: 18px; font-weight:bold;' cellpadding = 1px>
		<tr style='width: 450px;'>
			...
			<td style='vertical-align:middle;'> <span style='color: #000;'>" . __('Technician', 'paineldebordo') . ": </span>" . $row['firstname'] . " " . $row['realname'] . "</td>
			<td style='vertical-align:middle; ' colspan=2> <span style='color: #000;'>" . __('Tickets', 'paineldebordo') . ": </span>" . $conta_cons . "</td>
			<td colspan='3' style='font-size: 18px; font-weight:bold; vertical-align:middle; width:200px;'><span style='font-size: 18px; color:#000;'>" . __('Period', 'paineldebordo') . ": </span> " . conv_data($data_ini2) . " a " . conv_data($data_fin2) . "
	...
	<tr align='left'>
        ...
        <td><span style='color: #000;'>" . __('Solved', 'paineldebordo') . ": </span><b>" . $solve . " </b></td>
        <td><span style='color: #000;'>" . __('Closed') . ": </span><b>" . $close . " </b></td>

		<td colspan=3 align='right' style='vertical-align:middle;'>
			<button class='btn btn-primary btn-sm' type='button' name='abertos' value='Abertos' onclick='location.href=\"rel_tecnico.php?con=1&stat=open&sel_tec=" . $id_tec . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Opened', 'paineldebordo') . " </button>
			<button class='btn btn-primary btn-sm' type='button' name='fechados' value='Fechados' onclick='location.href=\"rel_tecnico.php?con=1&stat=close&sel_tec=" . $id_tec . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Closed', 'paineldebordo') . " </button>
			<button class='btn btn-primary btn-sm' type='button' name='todos' value='Todos' onclick='location.href=\"rel_tecnico.php?con=1&stat=all&sel_tec=" . $id_tec . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('All', 'paineldebordo') . " </button>
		</td>
	</tr>
	</table>
	...
	<table id='tec' class='display' style='font-size: 12px; font-weight:bold;' cellpadding = 2px >
		<thead>
			<tr>
				<th style='text-align:center; cursor:pointer;'> " . __('Tickets', 'paineldebordo') . " </th>
				...
				<th style='text-align:center; cursor:pointer;'> " . __('Opened', 'paineldebordo') . "</th>
				<th style='text-align:center; cursor:pointer;'> " . __('Closed', 'paineldebordo') . "</th>
				<th style='text-align:center; cursor:pointer;'> " . __('Time') . "</th>
				<th style='text-align:center;'> " . __('Satisfaction', 'paineldebordo') . "</th>
			</tr>
		</thead>
	<tbody>
	";
...
<table style='font-size: 16px; font-weight:bold; width: 50%;' border=0>
	...
	<table id='tec' class='display' style='font-size: 13px;' cellpadding = 2px >
		<thead>
			<tr>
				<th style='text-align:center; cursor:pointer; vertical-align:middle; font-weight:bold;'> " . __('Tickets', 'paineldebordo') . " </th>
				...
				<th style='text-align:center; cursor:pointer; vertical-align:middle;'> " . __('Requester') . "</th>
				<th style='text-align:center; cursor:pointer; vertical-align:middle;'> " . __('Opened', 'paineldebordo') . "</th>
			   <th style='text-align:center; cursor:pointer; vertical-align:middle;'> " . __('Closed', 'paineldebordo') . "</th>
				<th style='text-align:center; cursor:pointer;'> " . __('Time') . "</th>
			</tr>
		</thead>
	<tbody>
	...
	AND glpi_tickets_users.type = 1 ";
		...
		WHERE glpi_ticketsatisfactions.tickets_id = " . $row['id'] . " ";
										...
										$result_satc = $DB->doQuery($query_satc);
		...
		<tr style='font-size:11px;'>
									...
									$(document).ready(function() {
										...
										var table = $('#tec').DataTable({
											...
											buttons: [{
												...
												{
													extend: "collection",
													text: "<?php echo __('Print', 'paineldebordo'); ?>",
													buttons: [{
															extend: "print",
															autoPrint: true,
															text: "<?php echo __('All', 'paineldebordo'); ?>",
															message: "<div id='print' class='fluid span12' style='margin-bottom: 25px; margin-left: -1px;'><table id='print_tb' class='fluid'  style='width: 80%; margin-left: 10%; font-size: 18px; font-weight:bold;' cellpadding = '1px'><td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> <?php echo __('Technician', 'paineldebordo'); ?> : </span><?php echo $tech; ?> </td> <td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> <?php echo  __('Tickets', 'paineldebordo'); ?> : </span><?php echo $conta_cons; ?></td><td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle; width:200px;'><span style='color:#000;'> <?php echo  __('Period', 'paineldebordo'); ?> : </span> <?php echo conv_data($data_ini2); ?> a <?php echo conv_data($data_fin2); ?> </td> </table></div>",
														},
														{
															extend: "print",
															autoPrint: true,
															text: "<?php echo __('Selected', 'paineldebordo'); ?>",
															message: "<div id='print' class='fluid span12' style='margin-bottom: 25px; margin-left: -1px;'><table id='print_tb' class='fluid'  style='width: 80%; margin-left: 10%; font-size: 18px; font-weight:bold;' cellpadding = '1px'><td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> <?php echo __('Technician', 'paineldebordo'); ?> : </span><?php echo $tech; ?> </td> <td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> <?php echo  __('Tickets', 'paineldebordo'); ?> : </span><?php echo $conta_cons; ?></td><td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle; width:200px;'><span style='color:#000;'> <?php echo  __('Period', 'paineldebordo'); ?> : </span> <?php echo conv_data($data_ini2); ?> a <?php echo conv_data($data_fin2); ?> </td> </table></div>",
															exportOptions: {
																modifier: {
																	selected: true
													...
													buttons: ["csvHtml5", "excelHtml5",
														{
															...
															pageSize: 'A4',
															message: "<?php echo __('Technician', 'paineldebordo'); ?> : <?php echo $tech . '  - '; ?> <?php echo  __('Tickets', 'paineldebordo'); ?> : <?php echo $conta_cons . '  - '; ?>  <?php echo  __('Period', 'paineldebordo'); ?> : <?php echo conv_data($data_ini2); ?> - <?php echo conv_data($data_fin2); ?>"
														}
													]
												}
											]
			...
			<div id='nada_rel' class='well info_box fluid col-md-12'>
				<table class='table' style='font-size: 18px; font-weight:bold;' cellpadding = 1px>
					<tr>
						<td style='vertical-align:middle; text-align:center;'> <span style='color: #000;'>" . __('No ticket found', 'paineldebordo') . "</td></tr>
					<tr></tr>
				</table>
			</div>\n";
							}
