...
function dropdown($name, array $options, $selected = null)
{
	...
	$dropdown .= '<option value="-1">' . __('Select a group', 'paineldebordo') . '</option>' . "\n";
...
function margins()
{
	...
	global $DB;
	$query_lay = "SELECT value FROM glpi_plugin_paineldebordo_config WHERE name = 'layout' AND users_id = " . $_SESSION['glpiID'] . " ";
	$result_lay = $DB->doQuery($query_lay);
...
<head>
	<title> GLPI - <?php echo __('Tickets', 'paineldebordo') . '  ' . __('Technician group') ?> </title>
			...
			<div id="charts" class="fluid chart">
				<div id="pad-wrapper">
					<div id="head-rel" class="fluid">
						...
						<div id="titulo_rel"> <?php echo __('Tickets', 'paineldebordo') . '  ' . __('Technician group') ?> </div>
						<div id="datas-tec" class="col-md-12 col-sm-12 fluid">
							<form id="form1" name="form1" class="form_rel" method="post" action="rel_tecnicos.php?con=1">
								...
								ORDER BY `name` ASC";
											...
											$result_techs = $DB->doQuery($sql_techs);
											$grp = $DB->fetchAssoc($result_techs);

											$res_techs = $DB->doQuery($sql_techs);
											$arr_techs = array();
											$arr_techs[0] = "" . __('All') . "";
...
ORDER BY fname ASC ";
							...
							$result_sats = $DB->doQuery($query_sats);
							...
							if ($id_techs != -1 && $id_techs != 0) {
								...
								$result_gname = $DB->doQuery($query_gname);
	...
	<table id='tec' class='display' style='font-size: 13px; font-weight:bold;' cellpadding = 2px >
		<thead>
			<tr>
				<th style='text-align:center; cursor:pointer;'> " . __('Technician', 'paineldebordo') . " </th>
				...
				<th style='text-align:center; cursor:pointer;'> " . __('Backlog', 'paineldebordo') . "</th>";

							if ($sats != '') {
								echo "<th style='text-align:center; '> " . __('Satisfaction', 'paineldebordo') . "</th>";
							}
							echo "</tr>
		</thead>
	<tbody>";
...
" . $grupo_tic1 . " ";
								...
								$result_total = $DB->doQuery($sql_total) or die("erro_total");
...
" . $grupo_tic1 . " ";
								...
								$result_sol = $DB->doQuery($sql_sol) or die("erro_sol");
...
" . $grupo_tic1 . " ";
								...
								$result_clo = $DB->doQuery($sql_clo) or die("erro_clo");
...
" . $grupo_tic1 . " ";
								...
								$result_sat = $DB->doQuery($query_sat) or die('erro_sat');
...
" . $grupo_tic1 . " ";
								...
								if ($conta_cons > 0) {
									...
									if ($barra_due > 80 and $barra_due <= 100) {
										$cor_due = "progress-bar-danger";
										$text_color_due = "#fff";
									}
									if ($barra_due > 51 and $barra_due <= 80) {
										$cor_due = "progress-bar-warning";
										$text_color_due = "#fff";
									}
									if ($barra_due > 20 and $barra_due <= 50) {
										$cor_due = " ";
										$text_color_due = "#fff";
									}
									if ($barra_due > 0 and $barra_due <= 20) {
