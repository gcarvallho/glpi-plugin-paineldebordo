...
<head>
	<title> GLPI - <?php echo __('Cost') . '  ' . __('by Entity', 'paineldebordo') ?> </title>
...
<body style="background-color: #e5e5e5; margin-left:0%;">
	...
	<div id='content'>
			...
			<div id="charts" class="fluid chart">
				<div id="pad-wrapper">
					<div id="head-rel" class="fluid">
						...
						<div id="titulo_rel"> <?php echo __('Cost') . '  ' . __('by Entity', 'paineldebordo') ?> </div>
		...
		ORDER BY `name` ASC ";
											...
											$result_ent = $DB->doQuery(trim($sql_ent));

											$arr_ent = array();
											$arr_ent[0] = "-- " . __('Select a entity', 'paineldebordo') . " --";
					...
					if (isset($_GET['con'])) {
						...
						if ($con == "1") {
							...
							if ($id_ent == "" || $id_ent == -1) {
								echo '<script language="javascript"> alert(" ' . __('Select a entity', 'paineldebordo') . ' "); </script>';
	...
	AND glpi_tickets.entities_id IN (" . ($ents ?? 0) . ")
	GROUP BY id
	ORDER BY id DESC ";
	...
	<table class='fluid'  style='font-size: 18px; font-weight:bold; width:100%;' cellpadding = 1px>
		<tr style='width: 450px;'>
			<td style='vertical-align:middle;'> <span style='color: #000;'>" . __('Entity', 'paineldebordo') . ": </span>" . $row['name'] . "</td>
		...
		<table align='right' style='margin-bottom:10px;'>
			<tr>
				<td colspan=3 style='vertical-align:bottom;'>
					<button class='btn btn-primary btn-sm' type='button' name='abertos' value='Abertos' onclick='location.href=\"rel_custo_ent.php?con=1&stat=open&sel_ent=" . $id_ent . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Opened', 'paineldebordo') . " </button>
					<button class='btn btn-primary btn-sm' type='button' name='fechados' value='Fechados' onclick='location.href=\"rel_custo_ent.php?con=1&stat=close&sel_ent=" . $id_ent . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Closed', 'paineldebordo') . " </button>
					<button class='btn btn-primary btn-sm' type='button' name='todos' value='Todos' onclick='location.href=\"rel_custo_ent.php?con=1&stat=all&sel_ent=" . $id_ent . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('All', 'paineldebordo') . " </button>
				</td>
			</tr>
		</table>
													...
													buttons: ["csvHtml5", "excelHtml5",
														{
															...
															message: "<?php echo __('Entity', 'paineldebordo'); ?> : <?php echo $ent_name . ' - '; ?> <?php echo  __('Tickets', 'paineldebordo'); ?> : <?php echo $conta_cons . '  - '; ?>  <?php echo  __('Total cost'); ?> : <?php echo number_format($total_cost, 2, ',', ' ') . ' - '; ?> <?php echo  __('Period', 'paineldebordo'); ?> : <?php echo conv_data($data_ini2); ?> - <?php echo conv_data($data_fin2); ?>"
														}
													]
												}
											]
