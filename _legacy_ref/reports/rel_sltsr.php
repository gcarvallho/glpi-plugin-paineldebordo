...
<head>
	<title> GLPI - <?php echo __('Tickets') . '  ' . __('by SLA', 'paineldebordo') ?> </title>
			...
			<div id="charts" class="fluid chart">
				<div id="pad-wrapper">
					<div id="head-lg" class="fluid">
						...
						<div id="titulo_rel">
							<?php echo __('Tickets', 'paineldebordo') . '  ' . __('by SLA', 'paineldebordo') ?> - <?php echo __('Time to resolve'); ?>
						</div>
						...
						ORDER BY name ASC ";
											...
											$result_loc = $DB->doQuery($sql_loc);

											$arr_sla = array();
											$arr_sla[0] = "-- " . __('Select a SLA', 'paineldebordo') . " --";
					...
					if ($con == "1") {
						...
						if ($id_sla == " " || $id_sla == 0) {
							echo '<script language="javascript"> alert(" ' . __('Select a SLA', 'paineldebordo') . ' "); </script>';
...
ORDER BY id DESC ";
						...
						$result_cons1 = $DB->doQuery($conta_cons1);
...
<table class='col-md-12 col-sm-12 fluid'  style='font-size: 18px; font-weight:bold;' cellpadding = 1px >
	<tr>
		...
		<td style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> " . __('Tickets', 'paineldebordo') . ": </span>" . $conta_cons . " </td>
		<td colspan='2'   style='font-size: 16px; font-weight:bold; vertical-align:middle; width:230px;'><span style='color:#000;'>" . __('Period', 'paineldebordo') . ": </span> " . conv_data($data_ini2) . " a " . conv_data($data_fin2) . "
		</td>
...
<table align='right' style='margin-bottom:10px;'>
	<tr>
		<td>
			<button class='btn btn-primary btn-sm' type='button' name='abertos' value='Abertos' onclick='location.href=\"rel_sltsr.php?con=1&stat=open&sel_sla=" . $id_sla . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Opened', 'paineldebordo') . " </button>
			<button class='btn btn-primary btn-sm' type='button' name='fechados' value='Fechados' onclick='location.href=\"rel_sltsr.php?con=1&stat=close&sel_sla=" . $id_sla . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('Closed', 'paineldebordo') . " </button>
			<button class='btn btn-primary btn-sm' type='button' name='todos' value='Todos' onclick='location.href=\"rel_sltsr.php?con=1&stat=all&sel_sla=" . $id_sla . "&date1=" . $data_ini2 . "&date2=" . $data_fin2 . "\"' <i class='icon-white icon-trash'></i> " . __('All', 'paineldebordo') . " </button>
		</td>
	</tr>
	<tr>
		<table border=0 width='400px' align='left' >
			<tr style='font-size: 16px; font-weight:bold; vertical-align:middle;'>
				<td> " . __('Within', 'paineldebordo')  . " : " . $w . " - (" . round(($w * 100) / $conta_cons, 0) . " %) </td>
				<td> " . __('Overdue', 'paineldebordo') . " : " . $v . " - (" . round(($v * 100) / $conta_cons, 0) . " %) </td>
			</tr>
			<tr><td>&nbsp;</td></tr>
		</table>
	</tr>
...
<table style='font-size: 16px; font-weight:bold; width: 70%;' border=0>
	<tr>
		...
		<td><span style='color: #000;'>" . __('Solved', 'paineldebordo') . ": </span>" . $solve . " </td>
...
<table id='sla' class='display'  style='font-size: 12px; font-weight:bold;' >
	<thead>
		<tr>
			<th style='font-size: 12px; font-weight:bold; text-align: center; cursor:pointer; vertical-align:middle;'> " . __('Tickets', 'paineldebordo') . " </th>
			...
			<th style='font-size: 12px; font-weight:bold; text-align: center; cursor:pointer; vertical-align:middle;'> " . __('Opened', 'paineldebordo') . "</th>
	...
	<tr style='font-weight:normal;'>
		...
		<td style='vertical-align:middle; text-align:center;'> " . time_ext($row['time']) . "</td> ";
								...
								if ($row['solvedate'] > $row['duedate']) {
									echo "<td style='vertical-align:middle; text-align:center; color:red;'> <span class='label label-danger'>" . __('Overdue', 'paineldebordo') . " </span></td>";
								} else {

									if (!isset($row['solvedate']) and $today > $row['duedate']) {
										echo "<td style='vertical-align:middle; text-align:center; color:red;'> <span class='label label-danger'>" . __('Overdue', 'paineldebordo') . " </span></td>";
									} else {
										echo "<td style='vertical-align:middle; text-align:center; color:green;'> <span class='label label-success'>" . __('Within', 'paineldebordo') . " </span></td>";
									}
								}
												...
												buttons: ["csvHtml5", "excelHtml5",
													{
														...
														message: "<?php echo __('SLA'); ?> : <?php echo $ent_name['name'] . '  -  '; ?> <?php echo  __('Tickets', 'paineldebordo'); ?> : <?php echo $conta_cons . '  -  '; ?> <?php echo  __('Period', 'paineldebordo'); ?> : <?php echo conv_data($data_ini2); ?> a <?php echo conv_data($data_fin2); ?>",
													}
												]
											}
										]
