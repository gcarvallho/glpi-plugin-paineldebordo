...
ORDER BY id ASC ";
	...
	<table id='tarefa' class='display' style='font-size: 13px; font-weight:bold;' cellpadding = 2px>
		<thead>
			<tr>
				...
				<th style='text-align:center; cursor:pointer; vertical-align:middle'> " . __('Due Date', 'paineldebordo') . "</th>
			</tr>
		</thead>
	<tbody>
	";
	...
	WHERE id = " . $row['projectstates_id'] . " ";
							...
							$result_stat = $DB->doQuery($sql_stat);
	...
	WHERE projects_id = " . $row['id'] . " ";
							...
							$result_task = $DB->doQuery($sql_task);
							...
							$res_timer = $DB->doQuery($sql_timer);
									...
									buttons: [{
										...
										},
										{
											...
											buttons: [{
													...
													message: "<div id='print' class='info_box fluid span12' style='margin-bottom:35px; margin-left: -1px;'><table id='print_tb' class='fluid'  style='width: 80%; margin-left: 10%; font-size: 18px; font-weight:bold;' cellpadding = '1px'><td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'>  </td> <td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'> <?php echo  _n('Task', 'Tasks', 2); ?> : </span><?php echo $conta_cons; ?></td><td colspan='2' style='font-size: 16px; font-weight:bold; vertical-align:middle; width:200px;'><span style='color:#000;'> <?php echo  __('Period', 'paineldebordo'); ?> : </span> <?php echo conv_data($data_ini2); ?> a <?php echo conv_data($data_fin2); ?> </td> </table></div>",
												},
												{
													extend: "print",
													autoPrint: true,
